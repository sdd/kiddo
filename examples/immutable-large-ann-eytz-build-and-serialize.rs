...
type Tree = ImmutableKdTree<f64, usize, Eytzinger<4>, 4, BUCKET_SIZE>;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut file = File::create("./examples/immutable-ann-test-tree-eytz-rkyv_08.rkyv")?;
    file.write_all(&buf)
        .expect("Could not write serialized rkyv to file");

    let file_size = file.metadata().unwrap().len().bytes();
    println!(
        "Serialized k-d tree to rkyv file 'immutable-ann-test-tree-eytz-rkyv_08.rkyv' ({}). File size: {:.2}",
        ElapsedDuration::new(start.elapsed()),
        file_size
    );
