...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let neighbour = kdtree.nearest_one::<SquaredEuclidean>(&query).unwrap();
    ...
    let nearest = &cities[neighbour.item as usize];
    ...
    let query = degrees_lat_lng_to_unit_sphere(0f32, 0f32);
    let dist = kilometres_to_unit_sphere_squared_euclidean(1000.0);
    let best_3 = kdtree.best_n_within::<SquaredEuclidean>(&query, dist, 3);
    let best_3 = best_3
        .into_iter()
        .map(|neighbour| (&cities[neighbour.item as usize].name))
        .collect::<Vec<_>>();
