...
type Tree = ImmutableKdTree<f32, usize, DonnellyPf<4, 64, 4, 4>, 4, BUCKET_SIZE>;
type ArchivedTree = ArchivedR8ImmutableKdTree<f32, usize, DonnellyPf<4, 64, 4, 4>, 4, BUCKET_SIZE>;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Performing {QUERY_POINT_QTY:?} random stem traversals...");

    let start = Instant::now();
    query_points.iter().for_each(|point| {
        black_box(tree.get_leaf_node_idx_unrolled(point));
    });
    println!(
        "Queries complete. ({})",
        ElapsedDuration::new(start.elapsed())
