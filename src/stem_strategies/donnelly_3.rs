...
pub fn calc_child_idx(
    ...
    // bump minor_level
    let t = minor_level + 1;
    let inc_major = (t == 3) as u32;
    minor_level = t - inc_major * 3;

    // update min_idx
    let old_min = min_idx;
    min_idx = (old_min << 1) + 1 + right;

    // update maj_idx and base_maj when boundary
    maj_idx += inc_major;
    base_maj = maj_idx << 3;

    // Candidate A: same block
    let same = base_maj + min_idx;

    // Candidate B: cross block
    let min_row_idx = old_min.wrapping_sub(3 - 1);
    let next = (base_maj + ((min_row_idx << 1) + 1 + right)) << 3;

    // Blend
    let m = 0u32.wrapping_sub(inc_major);
    (same & !m) | (next & m)
}
