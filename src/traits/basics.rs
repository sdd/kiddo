...
/// than 65,535 points, you could use a `u16`. All these types implement `Basics` by default.
///
/// If you only care about storing the points themselves, you can use `()` for the item type.
/// If you do this, you'll need to use a specific constructor for the [`KdTree`]
/// ([`KdTree::new_from_slice_no_items`]), and the `best_n_within` query method will be unavailable.
///
