...
unsafe fn get_best_from_dists_f64_avx512<A: Axis, T: Content, const B: usize>(acc: [A; B], items: [T; B], best_dist: &mut A, best_item: &mut T) {
    ...
    for chunk in acc.as_chunks_unchecked::<8>().iter() {
        ...
        let is_better_mask = _mm512_cmp_pd_mask(
            min_dists_v,
            chunk_v,
            _CMP_NLT_UQ
        );
