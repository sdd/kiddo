...
ImmutableKdTree<A, T, K, B>
{
        ...
        F: Fn(&[A; K], &[A; K]) -> A,
    {
        ...
        let is_left_child =
            usize::from(*unsafe { query.get_unchecked(split_dim) } < val);
...
mod tests {
    ...
    fn can_query_nearest_one_item() {
        let mut tree: ImmutableKdTree<AX, u32, 4, 8> = ImmutableKdTree::new();
    ...
    fn can_query_nearest_one_item_large_scale() {
        ...
        let mut tree: ImmutableKdTree<AX, u32, 4, 32> = ImmutableKdTree::with_capacity(TREE_SIZE);
        content_to_add
            .iter()
