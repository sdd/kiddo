#[doc(hidden)]
#[macro_export]
macro_rules! generate_fixed_nearest_n_within_unsorted {
    ($comments:tt) => {
        doc_comment! {
            concat!$comments,

            #[inline]
            pub fn nearest_n_within<D, R: AxisFixed>(&self, query: &[A; K], dist: R, max_items: std::num::NonZero<usize>, sorted: bool) -> Vec<NearestNeighbour<R, T>>
            where
                D: DistanceMetricFixed<A, K, R>,
            {
                if sorted || max_items < std::num::NonZero::new(usize::MAX).unwrap() {
                    if max_items <= std::num::NonZero::new(MAX_VEC_RESULT_SIZE).unwrap() {
                        self.nearest_n_within_stub::<D, SortedVec<NearestNeighbour<R, T>>>(query, dist, max_items.get(), sorted)
                    } else {
                        self.nearest_n_within_stub::<D, BinaryHeap<NearestNeighbour<R, T>>>(query, dist, max_items.get(), sorted)
                    }
                } else {
                    self.nearest_n_within_stub::<D, Vec<NearestNeighbour<R, T>>>(query, dist, 0, sorted)
                }
            }

            fn nearest_n_within_stub<D: DistanceMetricFixed<A, K, R>, H: ResultCollection<R, T>>(
                &self, query: &[A; K], dist: R, res_capacity: usize, sorted: bool
            ) -> Vec<NearestNeighbour<R, T>> {
            ...
            unsafe fn nearest_n_within_unsorted_recurse<D, RI: ResultCollection<R, T>>(
                &self,
                query: &[A; K],
                radius: R,
                curr_node_idx: IDX,
                split_dim: usize,
                matching_items: &mut RI,
                off: &mut [A; K],
                rd: R,
            ) where
                D: DistanceMetricFixed<A, K, R>,
