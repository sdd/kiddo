#[doc(hidden)]
#[macro_export]
macro_rules! generate_fixed_within_unsorted_iter {
    ($comments:tt) => {
        doc_comment! {
            concat!$comments,
            #[inline]
            pub fn within_unsorted_iter<D, R: AxisFixed>(
                &'a self,
                query: &'query [A; K],
                dist: R,
            ) -> WithinUnsortedIter<'a, R, T>
            where
                D: DistanceMetricFixed<A, K, R>,
            {
                ...
                let gen = Gn::new_scoped(move |gen_scope| {
                    let query_ref = &query;
                    unsafe {
                        self.within_unsorted_iter_recurse::<D, R>(
                            query_ref,
                            dist,
                            root_index,
                            0,
            ...
            unsafe fn within_unsorted_iter_recurse<'scope, D, R: AxisFixed>(
                &'a self,
                query: &[A; K],
                radius: R,
                curr_node_idx: IDX,
                split_dim: usize,
                mut gen_scope: Scope<'scope, 'a, (), NearestNeighbour<R, T>>,
                off: &mut [A; K],
                rd: R,
            ) -> Scope<'scope, 'a, (), NearestNeighbour<R, T>>
            where
                D: DistanceMetricFixed<A, K, R>,
            {
                if is_stem_index(curr_node_idx) {
                    ...
                    gen_scope = self.within_unsorted_iter_recurse::<D, R>(
                        query,
                        radius,
                        closer_node_idx,
                        next_split_dim,
