...
use crate::generate_fixed_within_unsorted_iter;
...
where
    usize: Cast<IDX>,
{
    generate_fixed_within_unsorted_iter!(
...
```rust
    ...
    let within = tree.within_unsorted_iter::<SquaredEuclidean, FxdR>(&[Fxd::from_num(1), Fxd::from_num(2), Fxd::from_num(5)], FxdR::from_num(10)).collect::<Vec<_>>();
...
mod tests {
    ...
    fn can_query_items_within_radius() {
        ...
        let result: Vec<_> = tree
            .within_unsorted::<Manhattan, FxdR>(&query_point, radius)
            .into_iter()
