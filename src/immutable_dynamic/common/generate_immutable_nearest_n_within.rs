#[doc(hidden)]
#[macro_export]
macro_rules! generate_immutable_dynamic_nearest_n_within {
    ($comments:tt) => {
        doc_comment! {
            concat!$comments,
            #[inline]
                ...
                D: DistanceMetric<A, K>,
            {
                if stem_idx >= self.stems.len() {
                    self.leaf_nearest_n_within::<D, R>(
                        query,
                        radius,
                        matching_items,
            ...
            fn leaf_nearest_n_within<D, R: ResultCollection<A, T>>(
                &self,
                query: &[A; K],
                radius: A,
                nearest: &mut R,
                ...
                D: DistanceMetric<A, K>,
            {
                ...
                point_slice.nearest_n_within::<D, R>(query, radius, nearest);
            }
        }
    };
}
