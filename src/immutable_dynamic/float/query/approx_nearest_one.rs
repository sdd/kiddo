...
use crate::generate_immutable_dynamic_approx_nearest_one;

macro_rules! generate_immutable_dynamic_approx_float_nearest_one {
    ($doctest_build_tree:tt) => {
        generate_immutable_dynamic_approx_nearest_one!((
...
```rust
    use kiddo::immutable_dynamic::float::kdtree::ImmutableKdTree;
...
{
    generate_immutable_dynamic_approx_float_nearest_one!(
    ...
    > ArchivedImmutableKdTree<A, T, K>
{
    ...
    let tree = unsafe { rkyv::archived_root::<ImmutableKdTree<f64, u63, 3, 32>>(&mmap) };"
    );
}
