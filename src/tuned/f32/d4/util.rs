use std::arch::x86_64::{
    _mm_add_ps, _mm_add_ss, _mm_extract_ps, _mm_load_ps, _mm_max_ps, _mm_min_ps, _mm_movehdup_ps,
...
#[allow(dead_code)]
pub(crate) fn distance_to_bounds_simd_f32_4d_squared_euclidean(
    p1: &PT,
    min_bound: &PT,
    max_bound: &PT,
) -> A {
    unsafe {
        let pt_mm = _mm_load_ps(p1.as_ptr());
        let max_mm = _mm_load_ps(max_bound.as_ptr());
...
#[allow(dead_code)]
pub(crate) fn clamp(val: A, min_bound: A, max_bound: A) -> A {
    if val < min_bound {
        min_bound
...
pub(crate) fn extend(min_bound: &mut PT, max_bound: &mut PT, point: &PT) {
    ...
    F: FnMut(&AA, &AA) -> bool,
{
    loop {
        ...
        // TODO: will need to write mirror_insertion_sort to get this working
        //       SEE ~/.rustup/toolchains/nightly-x86_64-unknown-linux-gnu/lib/rustlib/src/rust/library/core/src/slice/sort.rs:172
        // const MAX_INSERTION: usize = 10;
        // if target.len() <= MAX_INSERTION {
        //     insertion_sort(target, is_less);
        //     return;
        // }
