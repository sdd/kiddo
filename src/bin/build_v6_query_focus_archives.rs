...
const POINT_SEED: u64 = 0x5eed_0000_0000_0501;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    eprintln!(
        "building v6 query-focus archives: points={} queries={} prefix={} point_seed={} query_seed={}",
        point_count,
        query_count,
        prefix.display(),
        POINT_SEED,
    ...
    let start = Instant::now();
    let tree: EytzingerPfFarTree = KdTree::new_from_slice(&points).unwrap();
    eprintln!(
        "built Eytzinger PF-far tree in {:.0} ns",
        start.elapsed().as_nanos() as f64
    );
    drop(points);
    ...
    write_archive_bytes(
        "Eytzinger PF-far tree",
        &bytes,
        &archive_path(&prefix, "eytzinger-pf-far-tree"),
        start.elapsed().as_nanos() as f64,
    )?;
    drop(bytes);
    drop(tree);
