    ...
    LS: ConstructibleLeafStrategy<A, T, SS, K, B>,
{
        ...
        T: TryFrom<usize>,
    {
        ...
        Self::new_from_slice_with(source, |leaf_items: &mut Vec<T>, src_idx: usize| {
            let item = T::try_from(src_idx).map_err(|_| {
    ...
    fn new_from_slice_with<F>(
        source: &[[A; K]],
        mut push_item: F,
    ) -> Result<Self, ConstructionError>
    where
        F: FnMut(&mut Vec<T>, usize) -> Result<(), ConstructionError>,
    {
    ...
    fn new_from_slice_no_stems_with<F>(
        source: &[[A; K]],
        mut push_item: F,
    ) -> Result<Self, ConstructionError>
    where
...
mod tests {
    ...
    fn update_pivot_shifts_right_when_left_scan_hits_zero() {
        ...
        let pivot = TestTree::update_pivot(&source, &mut sort_index, 0, 1).unwrap();
