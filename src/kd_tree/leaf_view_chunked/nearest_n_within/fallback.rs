    ...
    R: ResultCollection<D::Output, NearestNeighbour<D::Output, T>>,
{
    leaf.with_dists_for_slice_wide::<D, _>(query_wide, |dists| {
        LeafView::<AX, T, K, B>::update_nearest_dists(dists, leaf.items(), dist, results);
    });
}

#[inline(always)]
pub(crate) fn nearest_n_within_with_query_wide_arena_fallback<AX, T, D, R, const K: usize>(
