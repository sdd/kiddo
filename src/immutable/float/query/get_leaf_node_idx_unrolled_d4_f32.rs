...
impl ArchivedR8ImmutableKdTree<f32, usize, Donnelly<4, 64, 4, 4>, 4, 2> {
    pub fn get_leaf_node_idx_unrolled(&self, query: &[f32; 4]) -> usize {
        ...
        let max_stem_minor_tri_level = Into::<i32>::into(self.max_stem_level).wrapping_shr(2);
...
impl ArchivedR8ImmutableKdTree<f32, usize, DonnellyPf<4, 64, 4, 4>, 4, 2> {
    pub fn get_leaf_node_idx_unrolled(&self, query: &[f32; 4]) -> usize {
        let stems_ptr = std::ptr::NonNull::new(self.stems.as_ptr() as *mut u8).unwrap();
        let mut stem_ordering = DonnellyPf::<4, 64, 4, 4>::new(stems_ptr);
