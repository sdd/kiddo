...
pub fn donnelly_traversal_branchless(c: &mut Criterion) {
    ...
    for depth in [5, 10, 15, 20, 25] {
        group.bench_with_input(
            ...
            |b, &depth| {
                b.iter_batched(
                    || generate_flattened_calls_branchless(depth, TRAVERSALS_PER_BENCHMARK),
                    |calls| {
                        for (curr_idx, is_right_child, minor_level) in calls {
                            black_box(donnelly_get_idx_v2_branchless(curr_idx, is_right_child, minor_level));
                        }
                    },
...
fn generate_flattened_calls_branchless(depth: usize, traversal_count: usize) -> Vec<(u32, bool, u32)> {
    let mut rng = StdRng::seed_from_u64(RNG_SEED);
    let mut calls = Vec::with_capacity(depth * traversal_count);

    for _ in 0..traversal_count {
        let mut current_idx = 0u32;
        let mut minor_level = 0u32;

        for _ in 0..depth {
            let is_right_child = rng.gen_bool(0.5);
            calls.push((current_idx, is_right_child, minor_level));
            // Pre-compute next state for realistic progression
            current_idx = donnelly_get_idx_v2_branchless(current_idx, is_right_child, minor_level);
            minor_level = (minor_level + 1) % LOG2_ITEMS_PER_CACHE_LINE;
        }
    }
